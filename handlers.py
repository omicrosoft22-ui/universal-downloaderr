...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not match:
        # If it's a private chat and no URL is detected, act as an AI chatbot
        if not is_group:
            ...
            async for chunk in get_ai_response_stream(user_id, text):
                full_text += chunk
                
                if not message_obj:
                    message_obj = await update.message.reply_text(full_text + " ⏳")
                    last_edit_time = time.time()
                else:
                    if time.time() - last_edit_time > 1.0:
                        try:
                            await message_obj.edit_text(full_text + " ⏳")
                            last_edit_time = time.time()
                        except Exception:
                            pass
            ...
            if message_obj:
                try:
                    await message_obj.edit_text(full_text, parse_mode=constants.ParseMode.MARKDOWN)
                except Exception:
                    await message_obj.edit_text(full_text) # Fallback if markdown fails
        return
...
async def api_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        msg = f"ℹ️ **Current TeraBox API Key:**\n`{current_key}`\n\nTo change it, use:\n`/api <new_key>`"
        await update.message.reply_text(msg, parse_mode=constants.ParseMode.MARKDOWN)
        return
        
    new_key = context.args[0]
    set_terabox_api_key(new_key)
    msg2 = f"✅ **TeraBox API Key updated successfully!**\nNew Key: `{new_key}`"
    await update.message.reply_text(msg2, parse_mode=constants.ParseMode.MARKDOWN)
