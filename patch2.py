...
old_ensure = re.search(r"def _ensure_data_file\(\):.*?except Exception:\n\s+pass", content, re.DOTALL)
if old_ensure:
    content = content.replace(old_ensure.group(0), auth_code_addition)
else:
    print("Error: Could not replace _ensure_data_file")
...
with open('handlers.py', 'w', encoding='utf-8') as f:
    f.write(content)
print("Patch applied to handlers.py!")
